...
function isFirstMonthOfQuarter(stringDate: string): boolean {
    const month = new Date(stringDate).getUTCMonth() + 1;
...
function printSummary() {
    ...
    for (let index = 0; index <= 3; index++) {
        console.log(`${index + 1}кв: Доход :${incomeTotal[index]} Начислено: ${incomeTotal[index] * 0.06} Оплачено: ${socialTotal[index] + taxTotal[index]}`);
    }
    const accruedYear = incomeTotal[3] * 0.06;
    const paidYear = socialTotal[3] + taxTotal[3];
    console.log(`Остаток за год: ${accruedYear - paidYear}`);
}
...
let taxTotal: quarterReport = [0, 0, 0, 0];
...
for (let index = 0; index < data.length; index++) {
    ...
    if (element._type == operationType.Tax) {
        const paidQuarter = quarter(element._date);
        if (isFirstMonthOfQuarter(element._date) && paidQuarter > 0) {
            const coveredQuarter = paidQuarter - 1;
            for (let i = 3; i >= coveredQuarter; i--) {
